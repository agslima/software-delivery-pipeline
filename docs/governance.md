...
| Release static risk gate by immutable digest | `.github/workflows/ci-release-gate.yml` -> `trivy-scan`; `.github/workflows/release-static-risk.yml` -> `codeql-release-analysis`, `trivy-release-scan` | Release blocks when OPA denies normalized Trivy, CodeQL, and VEX evidence for backend, worker, or frontend images |
...
| Non-falsifiable evidence retained for audit | Trivy, CodeQL, VEX, OPA, ZAP, SBOM, digest, and Kyverno outputs uploaded as workflow artifacts | Release artifacts `trivy-results-*`, `codeql-release-sarif`, `static-risk-results-*`, `zap-results`, `sbom-*`, `digest-*`, and GitOps artifact `kyverno-gitops-log` |
