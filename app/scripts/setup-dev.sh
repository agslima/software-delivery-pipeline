...
# random_hex prints a hex-encoded random string of 2*bytes characters for the given number of bytes; returns non-zero if no cryptographic random source is available.
random_hex() {
    local bytes=$1
...
# generate_secret writes a hex-encoded random secret to SECRETS_DIR/<filename> if the file is missing and sets the file mode to 644; the optional second argument specifies the number of random bytes (default 32).
generate_secret() {
    local filename=$1
    local bytes=${2:-32}
