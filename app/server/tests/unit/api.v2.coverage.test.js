...
describe('Unit: api/v2 coverage', () => {
  describe('audit.helpers', () => {
    ...
    it('safeAudit applies default redaction mode and logs sanitized payload', async () => {
      ...
      await safeAudit({ logEvent }, { eventType: 'x', metadata: { token: 'abc', nested: { a: 1 } } });
  ...
  describe('v2 request schemas', () => {
    it('auth login schema normalizes email and rejects weak password', () => {
      ...
      const valid = loginSchema.validate({ email: '  USER@EXAMPLE.COM ', password: 'StrongPass123' });
      expect(valid.error).toBeUndefined();
      expect(valid.value.email).toBe('user@example.com');

      const invalid = loginSchema.validate({ email: 'user@example.com', password: 'short' });
