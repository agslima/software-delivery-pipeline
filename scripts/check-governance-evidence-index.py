...
def extract_readme_claims(text: str) -> list[str]:
    ...
    claims = [
        normalize_text(line[2:]) for line in section_lines if line.startswith("- ")
    ]
    if not claims:
        raise SystemExit(
...
def parse_workflow_references(
    cell: str, *, row: EvidenceRow
    ...
    for match in matches:
        ...
        references.append(
            WorkflowReference(workflow_path=workflow_path, jobs=job_tokens)
        )
...
def load_workflow_jobs(workflow_path: pathlib.Path) -> dict[str, str]:
    ...
    if not job_lookup:
        raise SystemExit(
            f"Workflow {workflow_path.relative_to(ROOT)} has no parseable top-level jobs map."
        )
...
def validate_claim_coverage(
    readme_claims: list[str], evidence_rows: list[EvidenceRow]
) -> list[str]:
    """
...
def run_check(
    ...
    evidence_index_path: pathlib.Path = EVIDENCE_INDEX_PATH,
) -> None:
    """
    ...
    evidence_rows = extract_evidence_rows(
        evidence_index_path.read_text(encoding="utf-8")
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--evidence-index", type=pathlib.Path, default=EVIDENCE_INDEX_PATH
    )
