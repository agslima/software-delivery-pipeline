...
write_index_md() {
  ...
  declare -A iac_targets_with_counts
  ...
  while IFS=$'\t' read -r iac_target iac_crit_t iac_high_t iac_med_t iac_low_t; do
    ...
    iac_targets_with_counts["${iac_target}"]=1
  ...
  while IFS= read -r iac_target; do
    [[ -n "${iac_target}" ]] || continue
    if [[ -n "${iac_targets_with_counts[${iac_target}]:-}" ]]; then
      continue
    fi
    iac_count_row="${iac_counts[${iac_target}]:-0 0 0 0}"
    read -r iac_crit_t iac_high_t iac_med_t iac_low_t <<<"${iac_count_row}"

    # Fallback for Snyk IaC JSON variants that expose aggregate target names
    # (for example scan root paths) separately from issue-level target paths.
    if [[ "${iac_count_row}" == "0 0 0 0" ]] && (( iac_crit + iac_high + iac_med + iac_low > 0 )) && (( ${#iac_targets_with_counts[@]} == 0 )); then
      iac_crit_t="${iac_crit}"
      iac_high_t="${iac_high}"
      iac_med_t="${iac_med}"
      iac_low_t="${iac_low}"
