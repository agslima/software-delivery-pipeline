...
def github_anchor_for_heading(heading: str) -> str:
    ...
    slug = "".join(
        char
        for char in slug
        if char in {" ", "-", "_"} or not unicodedata.category(char).startswith("P")
    )
...
def command_heading_any(args: argparse.Namespace) -> int:
    ...
    if any(expected in headings for expected in args.expected):
        return 0
    fail(
        f"Missing expected markdown heading variants in {args.file}: {', '.join(args.expected)}"
    )
...
def command_link_any(args: argparse.Namespace) -> int:
    ...
    if any(expected in targets for expected in args.expected):
        return 0
    fail(
        f"Missing expected markdown link targets in {args.file}: {', '.join(args.expected)}"
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Markdown-aware assertions for governance scripts."
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    heading_parser = subparsers.add_parser(
        "heading-any", help="Assert that a file contains at least one heading."
    )
    heading_parser.add_argument("file", help="Markdown file to inspect.")
    heading_parser.add_argument(
        "expected", nargs="+", help="Accepted heading text variants."
    )
    heading_parser.set_defaults(func=command_heading_any)

    link_parser = subparsers.add_parser(
        "link-any", help="Assert that a file contains at least one link target."
    )
    link_parser.add_argument("file", help="Markdown file to inspect.")
    link_parser.add_argument(
        "expected", nargs="+", help="Accepted link target variants."
    )
    link_parser.set_defaults(func=command_link_any)

    anchor_parser = subparsers.add_parser(
        "anchor", help="Print the GitHub-style anchor slug for a heading."
    )
