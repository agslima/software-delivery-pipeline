...
def test_is_schema_impact_path_rejects_non_schema_paths():
    assert not check_migration_safety.is_schema_impact_path(
...
def test_destructive_findings_detects_drop_column(
    tmp_path: pathlib.Path, monkeypatch: pytest.MonkeyPatch
):
    repo_root = tmp_path
    migration_path = (
        repo_root / "app/server/src/infra/db/migrations/20260411_cleanup.js"
    )
...
"""
    ...
    def fake_run(*args, **kwargs):
        return subprocess.CompletedProcess(
            args=args[0], returncode=0, stdout=diff_output, stderr=""
        )
