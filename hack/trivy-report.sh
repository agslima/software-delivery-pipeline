...
if ! command -v trivy >/dev/null 2>&1; then
  ...
  curl -sfL https://raw.githubusercontent.com/aquasecurity/trivy/main/contrib/install.sh | sh -s -- -b /tmp/trivy-bin v0.69.3
...
run_trivy_scan() {
  ...
  if [[ $exit_code -ne 0 ]]; then
    echo "::error::${scan_name} failed with exit code ${exit_code}." >&2
    if [[ -s "$stderr_log" ]]; then
      echo "::error::${scan_name} stderr:" >&2
      sed 's/^/  /' "$stderr_log" >&2
    fi
    rm -f "$stderr_log"
    exit "$exit_code"
  ...
  if [[ ! -s "$report_file" ]]; then
    echo "::error::${scan_name} did not produce a non-empty JSON report: ${report_file}" >&2
    rm -f "$stderr_log"
    exit 1
  fi

  if ! jq -e . "$report_file" >/dev/null 2>&1; then
    echo "::error::${scan_name} produced invalid JSON: ${report_file}" >&2
    rm -f "$stderr_log"
    exit 1
  fi

  if [[ -s "$stderr_log" ]]; then
    echo "::warning::${scan_name} completed with stderr output (non-fatal):" >&2
    sed 's/^/  /' "$stderr_log" >&2
  fi
...
run_trivy_scan "trivy fs app" "$FS_REPORT" trivy fs app --format json --output "$FS_REPORT" --scanners vuln
run_trivy_scan "trivy config app" "$CONFIG_REPORT" trivy config app --format json --output "$CONFIG_REPORT"
